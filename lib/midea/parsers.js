...
function parseC1(data, analysisMethod) {
    ...
    // Sub-body 0x4B (Group 11) — indoor unit louver ist-angles in degrees.
    // msmart-ng Group11Response (#280): horizontal at byte 9, vertical at
    // byte 12. msmart's payload[N] maps 1:1 to our data[N] (Group 7 proves
    // the offset base is identical).
    if (sub === 0x4B && data.length >= 13) {
        out.horizontalLouverAngle = data[9];
        out.verticalLouverAngle = data[12];
        return out;
    }
...
const NEW_PROTOCOL_TAGS = {
    ...
    // 0x7e new-protocol temperature payload. Reverse-engineered from model
    // 22013279 traces (midea-local #572); its regular C0 temperature fields
    // are stale and the real values live here instead. Gated on that model
    // only — see parseNewProtocolTemperature().
    new_protocol_temperature: 0x007E,
};

// 0x7e new-protocol temperature payload layout (midea-local message.py).
const NEW_PROTOCOL_TEMPERATURE_MIN_LENGTH = 41;
const NEW_PROTOCOL_SETPOINT_OFFSET = 11.5;
const NEW_PROTOCOL_SETPOINT_MASK = 0x3F;
const NEW_PROTOCOL_SETPOINT_HALF_DEGREE_BIT = 0x40;
const NEW_PROTOCOL_MIN_VALID_TEMPERATURE = 10;
const NEW_PROTOCOL_MAX_VALID_TEMPERATURE = 40;
const NEW_PROTOCOL_LEGACY_SETPOINT_BYTE = 3;
const NEW_PROTOCOL_INDOOR_TEMPERATURE_BYTE = 40;
const NEW_PROTOCOL_INDOOR_TEMPERATURE_DECIMAL_BYTE = 41;

// Models whose C0 temperature fields are stale and must be superseded by the
// 0x7e new-protocol payload. midea-local restricts this to a single verified
// model (message.py NEW_PROTOCOL_TEMPERATURE_MODELS); other subtypes send the
// same tag with unrelated content, so decoding it there corrupts readings.
const NEW_PROTOCOL_TEMPERATURE_MODELS = new Set(["22013279"]);

/**
 * Decode setpoint + indoor temperature from a 0x7e new-protocol payload.
 * Returns `{ temperatureSetpoint, indoorTemperature, outdoorTemperature }`
 * on success or null when the payload is too short / out of range. Mirrors
 * midea-local `_parse_new_protocol_temperatures` (message.py:1106).
 *
 * @param {Buffer|number[]} data raw tag payload
 */
function parseNewProtocolTemperature(data) {
    if (!data || data.length <= NEW_PROTOCOL_TEMPERATURE_MIN_LENGTH) return null;
    const rawSetpoint = data[1];
    let targetTemperature = NEW_PROTOCOL_SETPOINT_OFFSET + (rawSetpoint & NEW_PROTOCOL_SETPOINT_MASK) / 2;
    if (rawSetpoint & NEW_PROTOCOL_SETPOINT_HALF_DEGREE_BIT) targetTemperature += 0.5;
    if (targetTemperature < NEW_PROTOCOL_MIN_VALID_TEMPERATURE || targetTemperature > NEW_PROTOCOL_MAX_VALID_TEMPERATURE) {
        // Payload variant where the legacy byte-3 mapping is still active.
        const fallback = (data[NEW_PROTOCOL_LEGACY_SETPOINT_BYTE] - 50) / 2;
        if (fallback >= NEW_PROTOCOL_MIN_VALID_TEMPERATURE && fallback <= NEW_PROTOCOL_MAX_VALID_TEMPERATURE) {
            targetTemperature = fallback;
        } else {
            return null;
        }
    }
    const indoorTemperature = Math.round(
        ((data[NEW_PROTOCOL_INDOOR_TEMPERATURE_BYTE] - 50) / 2 + data[NEW_PROTOCOL_INDOOR_TEMPERATURE_DECIMAL_BYTE] * 0.1) * 10,
    ) / 10;
    if (indoorTemperature < NEW_PROTOCOL_MIN_VALID_TEMPERATURE || indoorTemperature > NEW_PROTOCOL_MAX_VALID_TEMPERATURE) {
        return null;
    }
    // Outdoor temperature isn't available locally on this model (the app
    // shows a cloud/weather value); null it so the bogus C0 value is dropped.
    return { temperatureSetpoint: targetTemperature, indoorTemperature, outdoorTemperature: null };
}
...
function parseBX(data) {
    ...
    // 0x7e temperature payload: expose the raw bytes so the device layer —
    // which knows the model number — can decide whether to trust it. Decoding
    // unconditionally would corrupt readings on models that reuse this tag
    // for unrelated content (midea-local #567/#563).
    if (params[NEW_PROTOCOL_TAGS.new_protocol_temperature]) {
        status._newProtocolTemperatureRaw = Buffer.from(params[NEW_PROTOCOL_TAGS.new_protocol_temperature]);
    }
    return status;
}
...
module.exports = {
    ...
    parseFDResponse,
    parseFrame,
    parseNewProtocolTemperature,
    NEW_PROTOCOL_TAGS,
    NEW_PROTOCOL_TEMPERATURE_MODELS,
    POWER_FORMATS,
};
